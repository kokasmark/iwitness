    ...
    render(){
        return (
            <Marker className="marker-click" key={"test"} coordinates={this.state.coordinates} onClick={() => this.expand(this)} onMouseOver={()=>this.setState({hover: true})} onMouseLeave={()=>this.setState({hover: false})}>
                {this.filterMarker() &&<circle className="marker-child" r={(1+(0.1*this.state.votes[0])+(-0.1*this.state.votes[1]))/this.props.parent.state.mapScaleFactor} fill="#F00" stroke="#fff" strokeWidth={1/this.props.parent.state.mapScaleFactor} articledata ="title"/>}
